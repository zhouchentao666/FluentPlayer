...
@media (max-width: 820px) {
  ...
  /* 移动端隐藏底部小封面（全屏播放器内已有大封面） */
  .player-footer :deep(.cover-placeholder) {
    display: none;
  }

  .player-footer :deep(.song-title) {
    text-align: center;
    font-size: 17px;
    font-weight: 700;
  }

  .player-footer :deep(.song-artist) {
    text-align: center;
    font-size: 13px;
  }

  .player-footer :deep(.play-btn) {
    width: 60px;
    height: 60px;
  }

  .player-footer :deep(.play-btn svg) {
    width: 30px;
    height: 30px;
  }

  .player-footer :deep(.control-btn),
  .player-footer :deep(.side-btn) {
    width: 48px;
    height: 48px;
  }

  .player-footer :deep(.control-btn svg),
  .player-footer :deep(.side-btn svg) {
    width: 26px;
    height: 26px;
  }
}
