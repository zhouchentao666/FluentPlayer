...
import type { Playlist, PlaylistTag } from '@online/lib/playlists'
import { getHotPlaylists, getPlaylistTags } from '@online/lib/playlists'
import OnlinePlatformTabs from './OnlinePlatformTabs.vue'
import OnlineCard from './OnlineCard.vue'

const emit = defineEmits<{ (e: 'open', item: Playlist): void }>()
...
async function loadTags() {
  try {
    const t = await getPlaylistTags(platform.value)
...
async function reload(p = 1) {
  ...
  page.value = p
  try {
    items.value = await getHotPlaylists(platform.value, p, tagId.value || null)
  } catch (e: unknown) {
...
<template>
  <div class="hot-view">
    ...
    <div v-else-if="items.length === 0" class="state">暂无歌单</div>
    ...
    <div v-else class="card-grid">
      <OnlineCard
        ...
        :item="it"
        kind="playlist"
        @open="emit('open', $event as Playlist)"
      />
    </div>
