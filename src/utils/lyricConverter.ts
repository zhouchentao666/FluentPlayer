...
import type { LyricInfo } from '@online/types/music'
...
// --- QRC-style absolute word-timed lyric re-packaging (ported from Museek) ---
// QQ 音乐的 QRC 解密后是 `LyricContent` 嵌套格式 `[行,时长](相对偏移,时长)字...`，
// 不能直接交给 AMLL 的 YRC/ESLRC 检测。这里把它重新编排成带【绝对时间】的 A2 行：
//   [mm:ss.SSS]<相对偏移,时长>字...
// 再交给 parseLrcA2 渲染（Museek 已验证可解析）。

function formatTimestampMs(timeMs: number): string {
  const t = Math.max(0, Math.floor(timeMs))
  const minutes = Math.floor(t / 60000)
  const seconds = Math.floor((t % 60000) / 1000)
  const milliseconds = t % 1000
...
function parseAbsoluteWordTimedBody(
  ...
  if (firstMarkerIndex > 0) {
    ...
    if (plainText) return { text: plainText, inline }
  }
...
/** 把 QRC 原文字节内 `[行,时长](偏移,时长)字...` 重新编排成绝对时间 A2 行。 */
export function parseAbsoluteWordTimedLyric(raw: string): LyricInfo {
  ...
  for (const rawLine of raw.split(/\r?\n/)) {
    ...
    const tag = formatTimestampMs(lineTime)
