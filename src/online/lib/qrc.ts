...
// 移植自 Museek-main：QQ 音乐 QRC 逐字歌词的 3DES-ECB + zlib 解密实现（已验证正确）。
...
function bitnumIntl(value: number, position: number, target: number): number {
  return ((((value << position) >>> 0) & 0x80000000) >>> target) >>> 0;
}
...
// QRC 解密文本偶带 cqqxmss 前缀与数字混淆，去除之（与 Museek 行为一致）。
function decodeQrcName(input: string): string {
  if (input.startsWith("cqqxmss")) {
    return input.replace("cqqxmss", "").replace(/[0-9]/g, "");
  }
  return input;
}
...
export function decodeQrc(value: string): string {
  ...
  if (!decrypted) return "";
  try {
    return decodeQrcName(new TextDecoder().decode(pako.inflate(decrypted)));
  } catch {
    return "";
  }
}

/** 从解密后的 QRC 文本中提取 LyricContent 包裹的逐字歌词 */
export function extractQrcContent(text: string): string {
  const match = text.match(/LyricContent="([\s\S]*?)"/);
  if (match) {
    return match[1];
  }
  return text;
